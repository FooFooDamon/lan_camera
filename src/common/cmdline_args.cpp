...
#define COPYRIGHT_STRING                "Copyright (c) 2026 Man Hung-Coeng <udc577@126.com>\n" \
                                        "Licensed under the Apache License, Version 2.0"
#endif

#ifndef BRIEF_INTRO
#define BRIEF_INTRO                     "Camera running on Local Area Network"
#endif
...
#define DEFAULT_CONF_FILE               "lan_camera.json"
#endif
#endif

#ifndef DEFAULT_PLAY_DIR
#define DEFAULT_PLAY_DIR                "/opt/lan_camera/save/server"
#endif
...
#define LOG_LEVEL_CANDIDATES            "config,debug,info,notice,warning,error"
#endif

#ifndef LOG_LEVEL_DEFAULT
#define LOG_LEVEL_DEFAULT               "config"
#endif

#endif // #ifdef HAS_LOGGER

#ifndef FPS_CANDIDATES
#define FPS_CANDIDATES                  "-1,10,15,20,25,30"
#endif
...
/*
 * NOTE: You can re-implement this function somewhere to get a real revision.
 */
__attribute__((weak))
const char *get_private_revision(void)
{
    return "<none>";
}
    ...
    const struct
    {
        ...
        },*/
        {
            ...
            " {" LOG_LEVEL_CANDIDATES "}\n\t\t\tSpecify log level."
                "\n\t\t\tDefault to " LOG_LEVEL_DEFAULT " (level value specified in config file)."
        },
#else
        {
        ...
        },
        {
            ...
            " {" FPS_CANDIDATES "}\n\t\t\tSpecify frame rate (frames-per-second)."
                "\n\t\t\tDefault to " FPS_DEFAULT " (auto selected)."
        },
        {
            { "show-sizes", no_argument, nullptr, 'S' },
            "\tShow image/canvas sizes supported by configuration file."
        },
        {
            { "which-size", required_argument, nullptr, 'W' },
            " 0/{1,2,3,...}\n\t\t\tSelect one of sizes supported by -S option for capturing/playing."
                "\n\t\t\tDefault to 0 (size value specified in config file)."
        },
        {
            { "play-dir", required_argument, nullptr, 'p' },
            " /PATH/TO/VIDEO/DIRECTORY\n\t\t\tSpecify video directory. For player only."
                "\n\t\t\tDefault to " DEFAULT_PLAY_DIR "."
        },
    };
...
#endif
    result.fps = FPS_DEFAULT;
    result.which_size = 0;
    result.play_dir = DEFAULT_PLAY_DIR;
...
#endif
        else if (abbr_map["show-sizes"] == c)
            result.show_sizes = true;
        else if (abbr_map["which-size"] == c)
            result.which_size = atoi(optarg);
        else if (abbr_map["play-dir"] == c)
            result.play_dir = optarg;
...
#endif
        { "frame rate", args.fps.c_str(), FPS_CANDIDATES },
    };
